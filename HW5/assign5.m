...
confmat_res = zeros(numClass,numClasses*length(K));
for i = 1 : length(K)
...
%% E.2
[c_labels, ~] = myKMeansClustering(data, 5);
acc_array = zeros(1,5);
kmeans_acc = zeros(1,10);
for j = 1 : 10
    for i = 1 : 5
        gt_cluster = labels(labels == i);
        est_cluster = c_labels(labels == i);
        diff_mode = mode(gt_cluster - est_cluster);
        acc = length(est_cluster(est_cluster == (i - diff_mode)))/length(est_cluster);
        acc_array(i) = acc;
    end
    kmeans_acc(j) = mean(acc_array);
end
kmeans_acc = mean(kmeans_acc);